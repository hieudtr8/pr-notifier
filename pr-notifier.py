...
def get_api_base_url(repo_url_str):
    """Xác định API base URL cho public hoặc enterprise GitHub."""
...
def parse_repo_url(url):
    """Trích xuất owner và repo từ URL."""
    try:
...
def parse_pr_url(url):
    """Trích xuất owner, repo, và pr_number từ URL của một PR."""
    try:
        path_parts = urlparse(url).path.strip("/").split("/")
...
def is_pr_url(url):
    """Kiểm tra xem URL có phải là của một Pull Request không."""
    try:
...
def check_and_notify(api_base_url, owner, repo, pr_number, pr_title, commit_sha, headers):
    """Kiểm tra trạng thái của một commit và gửi thông báo nếu hoàn thành."""
    ...
    if total_checks == 0:
        return "pending", False # Chưa có check nào, chưa hoàn thành
    ...
    if len(completed_checks) == total_checks:
        ...
        return conclusion, True # Đã hoàn thành và gửi thông báo
    
    return "in_progress", False # Vẫn đang chạy

def monitor_single_pr(pr_url, headers, api_base_url):
    """Chế độ theo dõi một PR duy nhất, tự động phát hiện commit mới."""
    ...
    try:
        while True:
            ...
            # Nếu PR đã đóng/gộp thì dừng lại
...
def monitor_repository(repo_url, headers, api_base_url):
    """Chế độ theo dõi tất cả PR trong một repo."""
